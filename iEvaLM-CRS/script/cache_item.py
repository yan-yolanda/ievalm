...
def annotate(item_text_list):
    ...
    for attempt in Retrying(
        reraise=True, retry=retry_if_not_exception_type((openai.BadRequestError, openai.AuthenticationError,openai.APITimeoutError)),
        wait=my_wait_exponential(min=1, max=60), stop=(my_stop_after_attempt(8)), before_sleep=my_before_sleep
    ):
        with attempt:
            response = client.embeddings.create(
                model='text-embedding-ada-002', input=item_text_list, timeout=request_timeout
            )
...
if __name__ == '__main__':
    ...
    while len(item_ids) > 0:
        ...
        for embed_obj in batch_embeds:
            item_id = batch_item_ids[embed_obj.index]  # 使用 .index 属性
            with open(f'{save_dir}/{item_id}.json', 'w', encoding='utf-8') as f:
                json.dump(embed_obj.embedding, f, ensure_ascii=False)  # 使用 .embedding 属性
