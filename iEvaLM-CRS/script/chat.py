...
from openai import APITimeoutError, APIError, BadRequestError, AuthenticationError
    ...
    def __call__(self, retry_state: "RetryCallState") -> float:
        if retry_state.outcome is not None:
            try:
                ...
                # 检查是否是超时错误
                if exc is not None and isinstance(exc, openai.APITimeoutError):
                    return 5.0  # 超时时等待5秒
            except:
                pass
    ...
    for attempt in Retrying(
        ...
        with attempt:
            response = client.chat.completions.create(
                model='gpt-4o-mini', messages=[{"role": "user", "content": prompt}], temperature=0, max_tokens=128, stop='Recommender',
